 ...
 */
uint8_t _address = MPU9250_DEFAULT_ADDRESS;
uint8_t _mag_address = MPU9250_MAG_ADDRESS;
...
bool writeRegister(const uint8_t register_addr, const uint8_t value) {
    ...
    bool write_status = 0;
    i2c_start(_address); //open communication with
...
bool writeMagRegister(const uint8_t register_addr, const uint8_t value) {
    ...
    //send value to register
    bool write_status = 0;
    i2c_start(_mag_address); //open communication with
    i2c_write(register_addr);
    i2c_write(value);
    i2c_stop();
...
uint8_t readRegister(const uint8_t register_addr) {
    ...
    i2c_rep_start((WHOAMI_DEFAULT_VAL << 1 ) + I2C_READ);

    while(!i2c_readNak()) {
        data = i2c_read(1);    // receive a byte as character
    }
